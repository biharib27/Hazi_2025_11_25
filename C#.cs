...
namespace Listak
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // 1. feladat – Melyik listában van több páratlan szám?
            ...
            if (paratlanA > paratlanB)
                Console.WriteLine("  A szamsorA listában van több páratlan szám.");
            else if (paratlanB > paratlanA)
                Console.WriteLine("  A szamsorB listában van több páratlan szám.");
            else
                Console.WriteLine("  Mindkettőben ugyanannyi páratlan szám van.");
            Console.WriteLine();

            // 4. feladat – ujSzamsor létrehozása rendezés nélkül, összefésüléssel

            List<int> ujSzamsor = new List<int>();

            int iA = 0;
            int iB = 0;
            while (iA < szamsorA.Count && iB < szamsorB.Count)
            {
                if (szamsorA[iA] < szamsorB[iB])
                {
                    ujSzamsor.Add(szamsorA[iA]);
                    iA++;
                }
                else
                {
                    ujSzamsor.Add(szamsorB[iB]);
                    iB++;
                }
            }

            while (iA < szamsorA.Count)
                ujSzamsor.Add(szamsorA[iA++]);

            while (iB < szamsorB.Count)
                ujSzamsor.Add(szamsorB[iB++]);

            Console.WriteLine("4. feladat:");
            Console.Write("  Összefésült lista: ");
            foreach (int n in ujSzamsor)
                Console.Write(n + " ");
            Console.WriteLine();
            Console.WriteLine();

            // 7. feladat – L800 eltávolítása

            for (int i = tintapatronok.Count - 1; i >= 0; i--)
                if (tintapatronok[i].Contains("L800"))
                    tintapatronok.RemoveAt(i);

            Console.WriteLine("7. feladat:");
            Console.WriteLine("  Lista elemeinek száma törlés után: " + tintapatronok.Count);
            Console.WriteLine();

            // 9. feladat – leghosszabb azonos jelű sorozat
            ...
            Console.WriteLine("  Leghosszabb sorozat hossza: " + maxHossz);
            Console.WriteLine();
